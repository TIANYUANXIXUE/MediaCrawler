    ...
    async def search(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * weibo_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                #如果搜索结果为空，则跳过
                if not note_list:
                    utils.logger.info(f"[WeiboCrawler.search] search weibo keyword: {keyword}, result is empty,break")
                    break
...
def craete_ai_task(post_id_list):
    try:
        ...
        r = requests.post('http://127.0.0.1:80/opinion/ai/wb', json={"post_id_list": post_id_list})
