...
class TencentClient(AbstractApiClient):
    def __init__(
            self,
            timeout=10,
            ip_pool=None,
        ...
        self._host = "https://i.news.qq.com"
    ...
    async def get(self, uri: str, params=None, return_ori_content=False, **kwargs) -> Any:
        ...
        try:
            res = await self.request(
                  method="GET", return_ori_content=return_ori_content, url=uri,**kwargs)
            return res
          ...
          utils.logger.error(f"[TencentClient.get] 达到了最大重试次数，IP已经被Block，请尝试更换新的IP代理: {e}")
          raise Exception(f"[TencentClient.get] 达到了最大重试次数，IP已经被Block，请尝试更换新的IP代理: {e}")
    ...
    async def post(self, uri: str, data: dict, **kwargs) -> Dict:
        ...
        """
        encoded_params = urlencode(data)  # 将参数转换为 URL 编码格式
        self.headers['content-type'] = 'application/x-www-form-urlencoded'
        return await self.request(method="POST", url=f"{self._host}{uri}",
                                  data=encoded_params, **kwargs)
    ...
    async def pong(self) -> bool:
        ...
        """
        utils.logger.info("[TencentClient.pong] Begin to pong tieba...")
        try:
            uri = "/mo/q/sync"
            res: Dict = await self.get(uri)
            utils.logger.info(f"[TencentClient.pong] res: {res}")
            if res and res.get("no") == 0:
                ping_flag = True
            else:
                utils.logger.info(f"[TencentClient.pong] user not login, will try to login again...")
                ping_flag = False
        except Exception as e:
            utils.logger.error(f"[TencentClient.pong] Ping tieba failed: {e}, and try to login again...")
            ping_flag = False
        return ping_flag
    ...
    async def get_notes_by_keyword(
        ...
        for item in sec_list:
            if item.get('component') == 'pictext':
                news_list = item.get('newsList', [])
                for news in news_list:
                    # 在这里编写处理 newsList 中每个新闻的逻辑
                    # 例如：提取标题、链接等信息
    ...
    async def get_note_by_id(self, note_id: str) -> TiebaNote:
        ...
        """
        url = f"https://news.qq.com/rain/a/{note_id}"
        html= await self.get(url,return_ori_content=True)
        item = dict(id=note_id, url=url,html=html)
        return self._page_extractor.extract_note_detail(item)
