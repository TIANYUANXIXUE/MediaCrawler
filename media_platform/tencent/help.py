...
class TieBaExtractor:
    ...
    def extract_article_info(html_content: str) -> Dict:
        """
        提取 content-article 下的标题、作者、发布时间、省份以及正文内容
        Args:
            html_content: HTML 内容

        Returns:
            包含标题、作者、发布时间、省份和正文内容的字典
        """
        selector = Selector(text=html_content)
        title = selector.xpath("//div[@class='content-article']/h1/text()").get(default='').strip()
        author = selector.xpath("//div[@class='media-info']/div[2]/a/p[@class='media-name']/text()").get(default='').strip()
        publish_time =selector.xpath("//div[@class='media-info']/div[2]/p[@class='media-meta']/span[1]/text()").get(default='').strip()
        province =selector.xpath("//div[@class='media-info']/div[2]/p[@class='media-meta']/span[2]//text()").getall()[1].strip()
        content = selector.xpath("//div[@id='article-content']").get(default='').strip()
    ...
    def extract_search_note_list(self, page_content: List[Dict]) -> List[TiebaNote]:
        """
        ...
        for item in page_content:
            ...
            tieba_note = TiebaNote(
                ...
                desc=article_info.get("content", ""),
                note_url=item.get("url", ""),
                user_nickname=article_info.get("author", ""),
                publish_time=article_info.get("publish_time", ""),
                province=article_info.get("province", "")
            )
    ...
    def extract_note_detail(self, page_content: Dict) -> TiebaNote:
        """
        提取贴吧帖子详情
        Args:
            page_content:
        ...
        article_info = self.extract_article_info(html)
        tieba_note = TiebaNote(
            note_id=page_content.get("id", ""),
            title=article_info.get("title", ""),
            desc=article_info.get("content", ""),
            note_url=page_content.get("url", ""),
            user_nickname=article_info.get("author", ""),
            publish_time=article_info.get("publish_time", ""),
            ip_location=article_info.get("province", "")
        )
        return tieba_note
