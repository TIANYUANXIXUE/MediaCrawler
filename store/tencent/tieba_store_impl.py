...
from var import crawler_type_var, media_crawler_mongo_db_var, post_id_list_all_var
...
class TencentMongoStoreImplement(AbstractStore):
    async def store_content(self, content_item: Dict):
        mongo_db_conn: AsyncMongoDB = media_crawler_mongo_db_var.get()
        content = await mongo_db_conn.find_one("tencent_contents", {"note_id": content_item.get("note_id")})
        if content:
            await mongo_db_conn.delete_one("tencent_contents", {"_id": content.get("_id")})
        await mongo_db_conn.insert_one("tencent_contents", content_item)

        new_item = transform_save_content_item(content_item)
        post = await mongo_db_conn.find_one("post_tencent", {"third_party_post_id": new_item.get("third_party_post_id")})
        if post:
            await mongo_db_conn.delete_one("post_tencent", {"_id": post.get("_id")})
        else:
            post_id_list_all_var.get().append(new_item['post_id'])
        await mongo_db_conn.insert_one("post_tencent", new_item)

    async def store_comment(self, comment_item: Dict):
        mongo_db_conn: AsyncMongoDB = media_crawler_mongo_db_var.get()
        comment = await mongo_db_conn.find_one("tencent_comments", {"comment_id": comment_item.get("comment_id")})
        if comment:
            await mongo_db_conn.delete_one("tencent_comments", {"_id": comment.get("_id")})
        await mongo_db_conn.insert_one("tencent_comments", comment_item)

        new_item = transform_save_comment_item(comment_item)
        post_comment = await mongo_db_conn.find_one("post_comment_tencent", {"third_party_comment_id": new_item.get("third_party_comment_id")})
        if post_comment:
            await mongo_db_conn.delete_one("post_comment_tencent", {"_id": post_comment.get("_id")})
        await mongo_db_conn.insert_one("post_comment_tencent", new_item)
...
def transform_save_content_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item.get('user_id') or item.get('user_nickname'):
        doc["author"] = {
            "user_id": str(item.get("user_id") or ""),
            "username": str(item.get("user_nickname") or "")
        }
    if item.get('publish_time'):
        doc['publish_time'] = _str_to_datetime(item.get("publish_time"))
...
def _str_to_datetime(
    ...
    :param time_str: 时间字符串，如 "2024-03-13 19:40"
    :param fmt: 字符串格式，默认是 "%Y-%m-%d %H:%M"
    :param tz: 时区，默认是北京时间（UTC+8）
    :return: 带有时区信息的 datetime 对象
    """
    if tz is None:
        tz = timezone(timedelta(hours=8))  # 默认设置为北京时间
