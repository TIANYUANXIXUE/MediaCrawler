...
class TieBaStoreFactory:
    STORES = {
        ...
        "mongo": TencentMongoStoreImplement
    }
...
async def update_tieba_note(note_item: TiebaNote):
    ...
    """
    # 先移除 <style>...</style> 块（包括标签本身）
    html_cleaned = re.sub(r"<style.*?>.*?</style>", "", note_item.desc, flags=re.DOTALL)
    # 再移除所有 HTML 标签，只保留文本
    text_only = re.sub(r"<.*?>", "", html_cleaned).strip()
    note_item.desc = text_only
