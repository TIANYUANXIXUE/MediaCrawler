...
class XhsMongoStoreImplement(AbstractStore):
    async def store_content(self, content_item: Dict):
        ...
        content = await mongo_db_conn.find_one("xhs_contents", {"note_id": content_item.get("note_id")})
        if content:
            await mongo_db_conn.delete_one("xhs_contents", {"_id": content.get("_id")})
        await mongo_db_conn.insert_one("xhs_contents", content_item)

        new_item = transform_save_content_item(content_item)
        post = await mongo_db_conn.find_one("post_xhs", {"third_party_post_id": new_item.get("third_party_post_id")})
        if post:
            await mongo_db_conn.delete_one("post_xhs", {"_id": post.get("_id")})
        else:
            post_id_list_all_var.get().append(new_item['post_id'])
        await mongo_db_conn.insert_one("post_xhs", new_item)

    async def store_comment(self, comment_item: Dict):
        mongo_db_conn: AsyncMongoDB = media_crawler_mongo_db_var.get()
        comment = await mongo_db_conn.find_one("xhs_comments", {"comment_id": comment_item.get("comment_id")})
        if comment:
            await mongo_db_conn.delete_one("xhs_comments", {"_id": comment.get("_id")})
        await mongo_db_conn.insert_one("xhs_comments", comment_item)

        new_item = transform_save_comment_item(comment_item)
        post_comment = await mongo_db_conn.find_one("post_comment_xhs", {"third_party_comment_id": new_item.get("third_party_comment_id")})
        if post_comment:
            await mongo_db_conn.delete_one("post_comment_xhs", {"_id": post_comment.get("_id")})
        await mongo_db_conn.insert_one("post_comment_xhs", new_item)
...
def transform_save_content_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if item.get('time'):
        doc['publish_time'] = _ts_to_datetime(item.get("time"))
